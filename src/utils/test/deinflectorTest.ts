...
function hasTermReasons(deinflector: Deinflector, source: string, expectedTerm: string, expectedRule: string | undefined, expectedReasons: Array<string> | undefined) {
...
function test() {
    ...
    const deinflectionReasons = JSON.parse(readFileSync(path.join('src/utils/data/deinflect.json')).toString()) as object;
    ...
    for (const {valid, tests} of data) {
        for (const {source, term, rule, reasons} of tests) {
            const {has, reasons: actualReasons} = hasTermReasons(deinflector, source, term, rule, reasons);
